  ...
  if (Number.isNaN(id)) {
    return (
      <div className="rounded-xl border border-red-500/30 bg-red-500/10 px-6 py-8 text-red-300">
        ...
        <Link className="underline text-[#adc6ff]" to={`${base}/courses`}>
          danh sách khóa học
        </Link>
        .
      </div>
  ...
  if (error || !course) {
    return (
      <div className="space-y-4">
        <Link
          to={`${base}/courses`}
          className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-[#adc6ff]"
        >
          <ArrowLeft size={16} /> Quay lại danh sách
        </Link>
        <div className="rounded-xl border border-red-500/30 bg-red-500/10 px-6 py-8 text-red-300">
          <p className="font-semibold text-red-200 mb-2">Không tải được chi tiết lớp</p>
          <p className="text-sm text-red-300/90 leading-relaxed">{error || "Không có dữ liệu"}</p>
          <button
            type="button"
            onClick={() => void load()}
            className="mt-4 px-4 py-2 rounded-lg bg-slate-800 text-slate-200 text-sm hover:bg-slate-700"
          >
            Thử lại
          </button>
        </div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 max-w-5xl">
      <div>
        <Link
          to={`${base}/courses`}
          className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-[#adc6ff] mb-4"
        >
          <ArrowLeft size={16} /> Quay lại danh sách
        </Link>
      ...
      {joinCode ? (
        <div className="rounded-xl border border-[#4fdbc8]/25 bg-gradient-to-br from-[#131b2e] to-[#0d3a32]/40 p-6 space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <span className="text-3xl sm:text-4xl font-mono font-black tracking-[0.25em] text-[#4fdbc8]">
              {joinCode}
            </span>
            <button
              type="button"
              onClick={copyJoinCode}
              className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-800/80 text-sm text-slate-200 hover:bg-slate-700 border border-slate-600/50"
            >
              {copiedJoin ? <Check size={16} className="text-emerald-400" /> : <Copy size={16} />}
              {copiedJoin ? "Đã chép" : "Sao chép"}
            </button>
          </div>
          <ul className="text-sm text-slate-400 space-y-1.5 list-disc list-inside leading-relaxed">
            <li>
              <strong className="text-slate-300">Sinh viên:</strong> đăng nhập vai trò Sinh viên → nhập{" "}
              <strong className="text-slate-200">mã 6 ký tự</strong> hoặc{" "}
              <strong className="text-slate-200">mã học phần</strong> (code) đầy đủ trên thanh trên cùng → Tham
              gia.
            </li>
            <li>
              <strong className="text-slate-300">Giảng viên chủ nhiệm:</strong> là người tạo lớp (hoặc được gán
              chủ nhiệm) — vào lớp qua không gian{" "}
              <span className="text-slate-300">Giảng viên / Khóa học</span>, không cần mã này.
            </li>
            <li>
              <strong className="text-slate-300">Giảng viên phụ / hội đồng:</strong> thêm ở mục bên dưới (ID tài
              khoản GV).
            </li>
          </ul>
        </div>
      ...
      <section
        ...
        className="rounded-xl border border-slate-800 bg-[#131b2e] p-6 space-y-4 scroll-mt-24"
      >
        <h3 className="font-bold text-[#dae2fd] text-lg">Cập nhật &amp; lưu trữ lớp</h3>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div className="sm:col-span-2">
              ...
              <input
                ...
                className="mt-1 w-full rounded-lg bg-[#0b1326] border border-slate-700 px-3 py-2 text-sm text-slate-200"
              />
            </div>
            <div>
              ...
              <input
                ...
                className="mt-1 w-full rounded-lg bg-[#0b1326] border border-slate-700 px-3 py-2 text-sm"
              />
            </div>
            <div>
            ...
            {canEditThesisType ? (
              ...
              <div className="sm:col-span-2 text-sm text-slate-400">
                Loại lớp: <span className="text-slate-200">khóa luận (thesis)</span> — chỉ quản lý/Admin đổi
                loại.
              </div>
            ) : (
              <div className="sm:col-span-2">
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                rows={3}
                className="mt-1 w-full rounded-lg bg-[#0b1326] border border-slate-700 px-3 py-2 text-sm min-h-[72px]"
              />
            </div>
          </div>
          <div className="flex flex-wrap gap-3 justify-between items-center pt-2 border-t border-slate-800/80">
            <button
              type="button"
              onClick={() => void handleDeleteCourse()}
              disabled={deletingCourse}
      ...
      <section className="rounded-xl border border-slate-800 bg-[#131b2e] p-6 space-y-4">
        <h3 className="font-bold text-[#dae2fd] flex items-center gap-2">
          <Users size={20} className="text-[#4fdbc8]" />
          Sinh viên &amp; duyệt ghi danh
        </h3>
        <p className="text-xs text-slate-500">
          Đã duyệt vào lớp: <strong className="text-[#dae2fd]">{activeForCap}</strong> / {maxCap}
          {pendingCount > 0 ? (
        ...
        <form onSubmit={handleAddStudent} className="flex flex-wrap gap-2 items-end">
          <div>
            ...
            <input
              ...
              className="block mt-1 rounded-lg bg-[#0b1326] border border-slate-700 px-3 py-2 text-sm text-slate-200 min-w-[240px]"
        ...
        <div className="grid md:grid-cols-3 gap-3 items-end">
          <div>
            ...
            <input
              ...
              className="block mt-1 w-full rounded-lg bg-[#0b1326] border border-slate-700 px-3 py-2 text-sm text-slate-200"
            />
          </div>
          <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-slate-500 border-b border-slate-800">
                ...
                <th className="p-3">Điểm tổng kết</th>
                <th className="p-3 text-right">Thao tác</th>
              </tr>
            </thead>
            <tbody>
              {filteredStudents.map((s) => (
                <tr key={s.enrollment_id} className="border-b border-slate-800/60">
                  <td className="p-3">
                    <input
                      type="checkbox"
                  ...
                  <td className="p-3 text-[#dae2fd]">{s.full_name}</td>
                  ...
                  <td className="p-3">
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        placeholder="Grade"
                        className="w-20 rounded bg-[#0b1326] border border-slate-700 px-2 py-1 text-xs"
                      />
                      <input
                        ...
                        max={10}
                        step="0.01"
                        className="w-24 rounded bg-[#0b1326] border border-slate-700 px-2 py-1 text-xs"
                      />
                      <button
                        type="button"
                        disabled={savingGradeId === s.enrollment_id}
                        onClick={() => void handleSaveEnrollmentGrade(s.enrollment_id)}
                        className="inline-flex items-center gap-1 px-2 py-1 rounded bg-slate-700 text-slate-200 text-xs hover:bg-slate-600 disabled:opacity-50"
                      >
        ...
        <form onSubmit={handleAddLecturer} className="flex flex-wrap gap-3 items-end">
          <div>
            ...
            <input
              ...
              className="block mt-1 rounded-lg bg-[#0b1326] border border-slate-700 px-3 py-2 text-sm w-32"
            />
          </div>
          <div>
            ...
            <select
              ...
              className="block mt-1 rounded-lg bg-[#0b1326] border border-slate-700 px-3 py-2 text-sm text-slate-200"
            >
          ...
          <button
            type="submit"
            className="px-4 py-2 rounded-lg bg-violet-600/80 text-white text-sm font-semibold"
          >
            Thêm
          </button>
        </form>
        <ul className="space-y-2">
          {assistants.map((a) => (
            <li
              ...
              className="flex items-center justify-between py-2 border-b border-slate-800/50 text-sm"
            >
              <span>
      ...
      <section className="rounded-xl border border-slate-800 bg-[#131b2e] p-6 space-y-4">
        <h3 className="font-bold text-[#dae2fd] flex items-center gap-2">
          <ClipboardList size={20} className="text-[#adc6ff]" />
        ...
        <form onSubmit={handleAddCriterion} className="flex flex-wrap gap-3 items-end">
          <input
            ...
            className="rounded-lg bg-[#0b1326] border border-slate-700 px-3 py-2 text-sm flex-1 min-w-[180px]"
          />
          <input
            type="number"
            step="0.01"
            ...
            className="rounded-lg bg-[#0b1326] border border-slate-700 px-3 py-2 text-sm w-28"
          />
          <button
            type="submit"
            className="px-4 py-2 rounded-lg bg-[#0566d9] text-white text-sm font-semibold"
          >
            Thêm tiêu chí
          </button>
        </form>
        <ul className="space-y-2">
          {criteria.map((c, index) =>
            editingCrit?.id === c.id ? (
              <li
                key={c.id}
                className="flex items-center gap-2 py-2 border-b border-slate-800/50 text-sm"
              >
                <input
                  value={editingCrit.name}
                  onChange={(e) => setEditingCrit({ ...editingCrit, name: e.target.value })}
                  className="flex-1 rounded-lg bg-[#0b1326] border border-slate-700 px-3 py-1.5 text-sm text-slate-200 min-w-0"
                />
                <input
                  type="number"
                  step="0.01"
                  ...
                  className="w-20 rounded-lg bg-[#0b1326] border border-slate-700 px-2 py-1.5 text-sm text-slate-200"
                />
              ...
              <li
                key={c.id}
                className="flex items-center justify-between py-2 border-b border-slate-800/50 text-sm text-slate-300"
              >
                <span>
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="text-slate-500 hover:text-slate-200 p-1 disabled:opacity-30"
                    title="Đưa lên"
                  >
                    ↑
                  </button>
                  ...
                  <button
                    ...
                    className="text-slate-500 hover:text-[#adc6ff] p-1"
                    title="Sửa tiêu chí"
                  >
                    <Pencil size={14} />
                  </button>
